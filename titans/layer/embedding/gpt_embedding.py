...
class GPTEmbedding(nn.Module):
    ...
    def __init__(self,
        ...
        if num_token_types > 0:
            self.tokentype_embeddings = col_nn.Embedding(num_token_types, embedding_dim, dtype=dtype)
        else:
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None):
        ...
        if self.tokentype_embeddings is not None and token_type_ids is not None:
            x = x + self.tokentype_embeddings(token_type_ids)
        x = self.dropout(x)
